...
/*
 * 重置连接的命令处理器，包括清除命令和子状态，释放item等操作。
 *
 * @param c 连接结构体指针
 */
static void reset_cmd_handler(conn *c) {
    ...
    // 如果存在item，释放item并重置相关标志
    if (c->item != NULL) {
        // TODO: 有其他到达此处的方法吗？
        // SASL auth错误地使用了它。应该没有其他地方吧？
        if (c->item_malloced) {
            free(c->item);
            c->item_malloced = false;
        } else {
...
// items.c.
item *
limited_get_locked(const char *key, size_t nkey, LIBEVENT_THREAD *t, bool do_update, uint32_t *hv, bool *overflow) {
    item *it;
...
/*
 * 尝试读取协商的命令，确定客户端使用的协议类型（二进制或ASCII）。
 *
 * @param c 连接结构体指针
 * @return int 读取命令的结果，由后续协议特定的 try_read_command 函数处理
 */
static int try_read_command_negotiate(conn *c) {
    assert(c != NULL);
    ...
    // 输出调试信息
    if (settings.verbose > 1) {
        fprintf(stderr, "%d: 客户端使用 %s 协议\n", c->sfd,
                prot_text(c->protocol));
    }

    // 调用相应协议的 try_read_command 函数处理后续命令读取
...
/*
 * 从网络读取尽可能多的数据，处理缓冲区溢出和连接关闭。
 * 在读取之前，将未完成命令的残余部分（如果有）移到缓冲区的开头。
 *
 * 为了防止有人使用虚假数据使连接消耗所有可用内存，通过一些realloc后跳出并开始查看我已经得到的数据...
 *
 * @return 枚举类型 try_read_result
 */
static enum try_read_result try_read_network(conn *c) {
    ...
    if (c->rcurr != c->rbuf) {
        // 如果存在未完成命令的残余部分，将其移动到缓冲区的开头
        if (c->rbytes != 0) /* 否则没有要复制的内容 */
    ...
    while (1) {
        // TODO: 移动到 rbuf_* 函数？
        // 如果读取的数据超过了缓冲区的大小，并且 rbuf 是通过 malloc 分配的
        if (c->rbytes >= c->rsize && c->rbuf_malloced) {
            // 防止由于恶意数据导致连接耗尽所有可用内存，限制 realloc 的次数
            if (num_allocs == 4) {
                return gotdata;
            }
            ++num_allocs;

            // 尝试对 rbuf 进行扩展，大小为原来的两倍
            ...
            if (!new_rbuf) {
                ...
                if (settings.verbose > 0) {
                    fprintf(stderr, "无法重新分配输入缓冲区\n");
                }
                c->rbytes = 0; /* 忽略已读取的内容 */
                out_of_memory(c, "SERVER_ERROR 读取请求时内存不足");
                c->close_after_write = true;
                return READ_MEMORY_ERROR;
            }

            // 更新 rcurl 和 rbuf 的指针，以及 rsize 的大小
        ...
        if (res > 0) {
            ...
            // 设置 gotdata 为 READ_DATA_RECEIVED 表示成功读取数据
            gotdata = READ_DATA_RECEIVED;
            c->rbytes += res;

            // 如果读取的字节数等于可用空间，并且 rbuf 是通过 malloc 分配的，则继续尝试扩展
        ...
        // 处理读取返回值为 0 的情况，表示连接正常关闭
        if (res == 0) {
            c->close_reason = NORMAL_CLOSE;
            return READ_ERROR;
        }

        // 处理读取返回值为 -1 的情况，可能是 EAGAIN 或 EWOULDBLOCK，表示需要再次尝试读取
        if (res == -1) {
...
static enum transmit_result transmit(conn *c) {
    ...
    // 调用 _transmit_pre 准备传输数据
    iovused = _transmit_pre(c, iovs, iovused, TRANSMIT_ALL_RESP);
    if (iovused == 0) {
        // 如果iovused为0，表示只处理noreply，直接返回传输完成
        _transmit_post(c, 0);
    ...
    if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
        // 如果是EAGAIN或EWOULDBLOCK，更新事件并返回传输暂时错误
...
// 判断是否支持 accept4 函数，如果支持则设置 use_accept4 为 1，否则为 0
    ...
    while (!stop) {
        ...
        case conn_parse_cmd:
            c->noreply = false;  // 重置标志，表示响应需要发送

        // 调用try_read_command函数尝试解析命令
        if (c->try_read_command(c) == 0) {
            /* we need more data! */
            // 需要更多数据，可能是因为命令还没有完全接收
            if (c->resp_head) {
                // 有缓冲的响应等待发送，此时先切换到conn_mwrite状态进行响应发送
                conn_set_state(c, conn_mwrite);
            } else {
                // 没有缓冲的响应，切换到conn_waiting状态等待更多数据
                conn_set_state(c, conn_waiting);
            }
        }
        ...
        // 处理连接状态为 conn_new_cmd 的情况，包括处理请求和根据条件切换状态。
        case conn_new_cmd:
            /* 一次只处理 nreqs 个请求，以避免饥饿其他连接 */
            --nreqs;
        if (nreqs >= 0) {
            // 处理新的请求
            reset_cmd_handler(c);
    ...
    case conn_mwrite:
        ...
        // 处理连接状态为 conn_mwrite 的情况，包括提交IO队列和传输数据。在传输完成后，根据连接的状态进行状态切换，并释放相关资源。
        for (io_queue_t *q = c->io_queues; q->type != IO_QUEUE_NONE; q++) {
            if (q->stack_ctx != NULL) {
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        return server_socket(settings.inter, port, transport, portnumber_file, ssl_enabled, 0,
